Template.tablesDynamic.rendered = function () {
    // dataTables
    if ($('.dataTable').length > 0) {
        $('.dataTable').each(function () {
            ...
            if ($(this).hasClass('dataTable-tools')) {
                ...
                opt.oTableTools = {
                    "sSwfPath": "/swf/datatable/copy_csv_xls_pdf.swf"
                };
            }
            if ($(this).hasClass("dataTable-scroller")) {
                ...
                opt.sDom = "frtiS";
                opt.sAjaxSource = "/json/datatable/demo.txt";
            }
...
Template.tablesLarge.rendered = function () {
    // dataTables
    if ($('.dataTable').length > 0) {
        $('.dataTable').each(function () {
            var opt = {
            ...
            if ($(this).hasClass("dataTable-scroller")) {
                ...
                opt.bDeferRender = true;
                opt.sDom = "frtiS";
                opt.sAjaxSource = "/json/datatables/demo.txt";
            }
            var oTable = $(this).dataTable(opt);
            $('.dataTables_filter input').attr("placeholder", "Search here...");
            $(".dataTables_length select").wrap("<div class='input-mini'></div>").chosen({
